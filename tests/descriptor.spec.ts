...
describe('Descriptor', function () {
    ...
    it('throws if incorrect value is given to directive', function () {
        // Arrange
        const scope = {} as ng.IScope;

        const attrs = {
            infiniteScroller: 'something of somecollection track by a',
        } as any as ng.IAttributes;

    // Act
    const creations = () => Descriptor.createFrom(scope, attrs);

    // Assert
    expect(creations).toThrow(`Expected expression in form of '_item_ in _collection_[ track by _id_]' but got 'something of somecollection track by a'.`);
});
...
describe('buffer size', function () {
    ...
    it('throws if incorrect value is given to bufferSize', function () {
        // Arrange
        const scope = {} as ng.IScope;
        const attr = {
            infiniteScroller: 'something in somecollection',
